...
def _find_normalized_gt(csv_dir: Path) -> Optional[Path]:
    """
    Try to locate a *_norm_offset*.csv written by Stage 5 in the same CSV folder.
    """
    try:
...
# ──────────────────────────────────────────────────────────────
# NEW: Per-threshold TP/FP/FN overlay helpers (Stage 5 outputs)
# ──────────────────────────────────────────────────────────────
...
def _find_stage5_dir_for_base(base: str, hints: List[Path]) -> Optional[Path]:
    """
    Best-effort discovery of Stage 5 output folder for a given video 'base'.
    ...
    cwd = Path.cwd()
    candidates.extend([
        cwd / 'stage5 validation' / base,
        cwd / 'stage5_validation' / base,
        cwd / 'stage5' / base,
    ])
    for c in candidates:
...
def _list_threshold_dirs(stage5_dir: Path) -> List[Tuple[str, Path]]:
...
def _render_tp_fp_fn_video_for_thr(
    *,
    ...
    if not cap.isOpened():
        raise RuntimeError(f"[stage6] Could not open video: {orig_video_path}")
    ...
    legend = "LEGEND_TP=YELLOW_FP=RED_FN=GREEN"
    out_path = out_dir / f"{base}_tp_fp_fn_{safe_thr}__{legend}.mp4"
    ...
    while True:
        ...
        out.write(frame)
        _progress(fr+1, limit, f"stage6-threshold-overlay:{thr_name}"); fr += 1

    cap.release(); out.release()
    print(f"[stage6] Wrote per-threshold TP/FP/FN video: {out_path}")
...
def overlay_gt_vs_model(
    *,
    ...
    # NEW — hint for locating Stage 5 per-threshold outputs (thr_* folders)
    stage5_dir_hint: Optional[Path] = None,
    # Backwards-compatible alias (older numbering)
    stage6_dir_hint: Optional[Path] = None,
    render_threshold_overlays: bool = True,
    thr_box_w: Optional[int] = None,
    thr_box_h: Optional[int] = None,
):
    """
    Stage 6 does TWO things now:
    ...
    2) NEW PER-THRESHOLD OVERLAYS:
       For each Stage 5 distance threshold folder (thr_*.px), renders a separate video
       where boxes are colored by classification:
         - TP = YELLOW
         - FP = RED
         - FN = GREEN
       These use the per-threshold fps.csv / tps.csv / fns.csv saved by Stage 5.
       Box sizes default to gt_box_w/h unless thr_box_w/h are provided.

    This stage expects that Stage 5 has run (for normalized GT and per-threshold CSVs).
    """
    if stage5_dir_hint is None and stage6_dir_hint is not None:
        stage5_dir_hint = stage6_dir_hint
    ...
    if gt_norm_csv_path is None:
        ...
        if gt_norm_csv_path is None:
            raise FileNotFoundError(
                "Could not auto-locate normalized GT CSV (expected *_norm_offset*.csv). Run Stage 5 first or pass gt_norm_csv_path."
            )
    ...
    while True:
        ...
        out.write(frame)
        _progress(fr+1, limit, 'stage6-overlay'); fr += 1
    ...
    if render_threshold_overlays:
        base = orig_video_path.stem
        # Try to locate the Stage 5 directory for this video
        stage5_dir = None
        if stage5_dir_hint is not None and stage5_dir_hint.exists():
            # Allow both the parent folder and the per-video child
            if stage5_dir_hint.name == base and stage5_dir_hint.is_dir():
                stage5_dir = stage5_dir_hint
            else:
                cand = stage5_dir_hint / base
                if cand.exists():
                    stage5_dir = cand
        if stage5_dir is None:
            ...
            candidates.append(csv_parent.parent / 'stage5 validation')
            candidates.append(Path.cwd() / 'stage5 validation')
            stage5_dir = _find_stage5_dir_for_base(base, candidates)

        if stage5_dir is None:
            print("[stage6] WARNING: Could not find Stage 5 per-threshold outputs; skipping TP/FP/FN videos.")
            return

        thr_dirs = _list_threshold_dirs(stage5_dir)
        if not thr_dirs:
            print(f"[stage6] WARNING: No threshold folders found in: {stage5_dir} — skipping TP/FP/FN videos.")
            return

        # Output folder for these videos: next to the main overlay video
        per_thr_out_dir = out_video_path.parent
        ...
        print(f"[stage6] Rendering TP/FP/FN videos for {len(thr_dirs)} thresholds from: {stage5_dir}")
