...
def _analyze_threshold_dir_and_render(
    *,
    ...
    out_csv = output_thr_dir / 'fp_nearest_tp.csv'
    out_img_dir = output_thr_dir / 'fp_pair_frames'
    out_img_dir.mkdir(parents=True, exist_ok=True)

    # NEW: extra folder for FP vs all GT overlays
    out_img_dir_fp_vs_gt = output_thr_dir / 'fp_vs_gt_frames'
    ...
    if not cap.isOpened():
        raise RuntimeError(f"[stage8] Could not open video: {orig_video_path}")
        ...
        while True:
            ...
            if fr in per_t_pairs and per_t_pairs[fr]:
                for rec in per_t_pairs[fr]:
                    ...
                    _progress(done, total_fps, f"stage8@{thr_dir.name}")
...
def stage8_fp_nearest_tp_analysis(
    *,
    ...
    ----------
    stage5_video_dir : Path
        Folder with Stage-5 outputs for a single video (contains thr_* subdirs).
    orig_video_path : Path
    ...
    if not stage5_video_dir.exists():
        if verbose:
            print(f"[stage8] Stage-5 dir not found: {stage5_video_dir}")
        return

    thr_dirs = sorted([p for p in stage5_video_dir.iterdir() if p.is_dir() and p.name.startswith('thr_')])
    if not thr_dirs:
        if verbose:
            print(f"[stage8] No thr_* folders in: {stage5_video_dir}")
        return

    if verbose:
        print(f"[stage8] Found {len(thr_dirs)} threshold folders under: {stage5_video_dir}")
    ...
    for td in thr_dirs:
        ...
        if not (fps_csv.exists() and tps_csv.exists()):
            if verbose:
                print(f"[stage8] Skipping {td.name} (missing fps.csv or tps.csv)")
            continue
    ...
    if verbose:
        print(f"[stage8] Done. Total FPs analyzed: {total_fps_all}.  "
              f"FPs without any TP in the same frame: {total_no_tp}.")


# Backwards-compatible alias (older numbering)
stage9_fp_nearest_tp_analysis = stage8_fp_nearest_tp_analysis
