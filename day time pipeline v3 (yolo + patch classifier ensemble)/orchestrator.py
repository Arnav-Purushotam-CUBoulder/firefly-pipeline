...
from stage3_1_trajectories import run_for_video as stage3_1_run
...
def _print_stage_times(stage_times: dict[str, float]) -> None:
    keys = ["stage1", "stage2", "stage3", "stage3_1", "stage4", "stage5"]
    print("\nTiming summary:")
    for k in keys:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for vid in videos:
        ...
        if getattr(params, "RUN_STAGE3_1_TRAJECTORIES", True):
            t0 = time.perf_counter()
            s3_traj_csv = stage3_1_run(vid)
            stage_times["stage3_1"] = time.perf_counter() - t0
            print(f"Stage3.1 Time: {stage_times['stage3_1']:.2f}s (csv: {s3_traj_csv.name})")
