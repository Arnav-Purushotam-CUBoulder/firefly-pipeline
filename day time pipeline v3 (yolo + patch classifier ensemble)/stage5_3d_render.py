...
def _load_stage3_detections(stem: str) -> List[Tuple[int, float, float]]:
    """Load Stage 3 detections for a video stem.

    Returns list of (frame_idx, x_center, y_center).
    """
...
def _group_into_blocks(
    dets: List[Tuple[int, float, float]], block_size: int
    ...
    """
    blocks: Dict[int, List[Tuple[int, float, float]]] = {}
    for t, cx, cy in dets:
        if t < 0:
            continue
        block_idx = t // block_size
        blocks.setdefault(block_idx, []).append((t, cx, cy))
    return blocks
...
def _render_block(
    stem: str,
    block_idx: int,
    block_dets: List[Tuple[int, float, float]],
    block_size: int,
    out_dir: Path,
    sphere_radius: float,
) -> Path:
    ...
    # Convert to arrays
    t_vals = np.array([t for (t, _, _) in block_dets], dtype=np.float32)
    x_vals = np.array([cx for (_, cx, _) in block_dets], dtype=np.float32)
    y_vals = np.array([cy for (_, _, cy) in block_dets], dtype=np.float32)
    ...
    # Build glyph spheres as a single PolyData
