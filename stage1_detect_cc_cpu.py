...
def _binary_mask(gray: np.ndarray, method: str, fixed_thr: int, open_ksize: int) -> np.ndarray:
    ...
    k = max(1, int(open_ksize));  k += (k % 2 == 0)  # make odd
    if k > 1:
...
def detect_stage1_to_csv(
    *,
    ...
    while True:
        ...
        mask = _binary_mask(gray, threshold_method, fixed_threshold, open_ksize)
        num, _, stats, _ = cv2.connectedComponentsWithStats(mask, connectivity=4 if connectivity==4 else 8)

        # stats columns: [x, y, w, h, area]
        for lbl in range(1, num):
            x, y, w, h, area = map(int, stats[lbl])
