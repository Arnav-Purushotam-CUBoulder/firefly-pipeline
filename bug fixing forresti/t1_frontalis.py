...
Notes:
  ...
  • (x, y) are treated as the **center** of the 10×10 crop.
...
INPUT_CSV   = Path('/Users/arnavps/Desktop/FFT resnet fixing FPs/20px frontalis inference data/csv files/fixed_to_center_val_9k-14k_frontalis_clip.csv')     # your CSV/TSV file
INPUT_VIDEO = Path('/Users/arnavps/Desktop/FFT resnet fixing FPs/20px frontalis inference data/raw input videos/val_9k-14k_frontalis_clip.mp4')           # your video file
OUTPUT_DIR  = Path("forresti, fixing FPs and box overlap/Proof of concept code/test1/bug fixing forresti/bug fixing data/output crops frontalis")        # where 10x10 crops will be written
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# CSV delimiter — set to '\t' for TSV (your example), or ',' for CSV
DELIMITER   = ','
...
# NEW: Inclusive frame ranges to process (CSV frame numbers). Example: [[0, 300], [1200, 1300]]
# If empty, process ALL frames.
FRAME_RANGES = []

# NEW: Logits output CSV (rows for the crops we actually saved)
OUTPUT_LOGITS_CSV = Path("forresti, fixing FPs and box overlap/Proof of concept code/test1/bug fixing forresti/bug fixing data/9k to 14k Hybrid FFT model OP Frontalis.csv")

# NEW: CNN model params for logits
MODEL_PATH   = Path('/Users/arnavps/Desktop/RA info/New Deep Learning project/TESTING_CODE/background subtraction detection method/actual background subtraction code/frontalis, tremulans and forresti global models/resnet18_FFT_best_model.pt')
BACKBONE     = 'resnet18'   # one of: resnet18, resnet34, resnet50, resnet101, resnet152
BACK_IDX     = 0
FIRE_IDX     = 1

# NEW: Offset to add to 't' (frame index) right before the FINAL save of OUTPUT_LOGITS_CSV
OUTPUT_T_OFFSET = 9000

# ─── helpers ───────────────────────────────────────────────────

def _center_crop_clamped(img: np.ndarray, cx: int, cy: int, w: int, h: int):
    """Return a clamped w×h crop centered at (cx,cy). Also returns x0,y0 (top-left in full image)."""
    H, W = img.shape[:2]
    x0 = int(round(cx - w / 2))
    y0 = int(round(cy - h / 2))
    if x0 < 0: x0 = 0
    ...
    x1 = min(x0 + w, W)
    y1 = min(y0 + h, H)
    # ensure output size by shifting back if needed
    ...
    with csv_path.open('r', newline='') as fh:
        ...
        for r in reader:
            ...
            try:
                ...
                continue
            by_frame.setdefault(fr, []).append((x, y))
            kept_rows += 1
...
def _crop_and_save_for_frame(frame_img: np.ndarray,
    ...
    """
    Given a frame image and a list of (x,y) centers, save 10x10 crops.
    Returns number of crops successfully saved.
    """
    saved = 0
    for (x, y) in points:
        crop, x0, y0 = _center_crop_clamped(frame_img, x, y, PATCH_W, PATCH_H)
        # If the frame is smaller than the patch in any dimension, skip
        if crop.shape[0] != PATCH_H or crop.shape[1] != PATCH_W:
            continue
        out_name = f"{FILENAME_PREFIX}frame_{frame_num:0{FRAME_ZPAD}d}_x{x}_y{y}.png"
...
def main():
    ...
    with torch.no_grad():
        for fr in target_frames_sorted:
            ...
            for (x, y) in points:
                crop_bgr, _, _ = _center_crop_clamped(frame, x, y, PATCH_W, PATCH_H)
                if crop_bgr.shape[0] != PATCH_H or crop_bgr.shape[1] != PATCH_W:
                    continue  # skip if crop can't be formed

                # save crop image (unchanged behavior)
                out_name = f"{FILENAME_PREFIX}frame_{fr:0{FRAME_ZPAD}d}_x{x}_y{y}.png"
