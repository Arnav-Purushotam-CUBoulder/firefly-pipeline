...
try:
    ...
    raise ImportError(
        "GPU CC variant requires CuPy + cuCIM (match CUDA): pip install 'cupy-cuda12x' 'cucim'"
    ) from e
...
def detect_stage1_to_csv(
    *,
    ...
    conn = 1 if int(connectivity) == 4 else 2  # cuCIM: 1=4-connectivity, 2=8-connectivity
    ...
    while True:
        ...
        # Threshold (CPU) then label on GPU
        mask_cpu = _binary_mask(
            gray,
            threshold_method,
            fixed_threshold,
            open_ksize,
