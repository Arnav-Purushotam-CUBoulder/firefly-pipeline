...
RUN_RESULTS_MANIFEST_FILENAME: str = "run_results_manifest.json"
...
ULTRALYTICS_WEIGHTS_CACHE_ROOT: Path = Path.home() / ".cache" / "firefly_pipeline" / "ultralytics_weights"
...
# Frontalis baseline predictions were generated against a clip variant with
# 19 leading frames relative to the shared annotation CSV. Shift GT later only
# for baseline scoring so the stored baseline artifacts line up without
# changing the shared raw annotation file or pipeline evaluation.
BASELINE_GT_T_SHIFT_BY_VIDEO_STEM: Dict[str, int] = {
...
def _night_pipeline_dir() -> Path:
    p = _repo_root() / "Pipelines" / "night_time_pipeline"
    if not (p / "stage9_validate.py").exists():
        raise FileNotFoundError(p / "stage9_validate.py")
    return p


_PY_MODULE_CACHE: Dict[str, Any] = {}


def _load_python_module(module_key: str, module_path: Path) -> Any:
    cached = _PY_MODULE_CACHE.get(module_key)
    ...
    _PY_MODULE_CACHE[module_key] = module
    return module
...
def _write_baseline_species_results_and_registry(
    *,
    ...
    baseline_gt_by_video_key: Dict[str, Dict[str, Any]],
    dry_run: bool,
) -> None:
    ...
    for video in eval_videos:
        ...
        gt_info = dict(baseline_gt_by_video_key.get(video.video_key) or {})
            ...
            methods_payload.setdefault(method_key, {}).setdefault(video.species_name, []).append(
                {
                    ...
                    "gt_source": str(gt_info.get("gt_source") or video.gt_source or ""),
                    "gt_rows": int(gt_info.get("normalized_gt_rows") or len(video.gt_rows)),
                    "gt_max_t": (
                        int(gt_info.get("normalized_gt_max_t"))
                        if gt_info.get("normalized_gt_max_t") is not None
                        else (int(video.gt_max_t) if video.gt_max_t is not None else None)
                    ),
                    "gt_processing_mode": str(gt_info.get("gt_processing_mode") or ""),
                    "gt_processing_route": str(gt_info.get("gt_processing_route") or video.route),
...
def _infer_gt_t_offset_from_rows(*, video_path: Path, gt_rows: Sequence[Dict[str, int]]) -> int:
    if not gt_rows:
        return 0
    video_frames = _video_frame_count(video_path)
    if video_frames is None:
        return 0
    min_t = min(int(r["t"]) for r in gt_rows)
    max_t = max(int(r["t"]) for r in gt_rows)
    if min_t > 0 and max_t >= video_frames:
        return int(min_t)
    return 0


def _baseline_gt_processing_config(route: str) -> Dict[str, Any]:
    route_name = _normalize_route(route)
    if route_name == "day":
        params_module = _load_python_module(
            "_tmp_baseline_day_params",
            _day_pipeline_dir() / "params.py",
        )
        validator_module = _load_python_module(
            "_tmp_baseline_day_stage5_validate",
            _day_pipeline_dir() / "stage5_validate.py",
        )
        return {
            "route": "day",
            "validator_module": validator_module,
            "crop_w": int(getattr(params_module, "STAGE5_CROP_W", getattr(params_module, "PATCH_SIZE_PX", 10))),
            "crop_h": int(getattr(params_module, "STAGE5_CROP_H", getattr(params_module, "PATCH_SIZE_PX", 10))),
            "gt_area_threshold_px": int(getattr(params_module, "STAGE5_GT_AREA_THRESHOLD_PX", 4)),
            "gt_bright_max_threshold": int(getattr(params_module, "STAGE5_GT_BRIGHT_MAX_THRESHOLD", 50)),
            "min_pixel_brightness_for_area_calc": int(
                getattr(params_module, "STAGE5_MIN_PIXEL_BRIGHTNESS_FOR_AREA_CALC", 50)
            ),
            "gt_dedupe_dist_threshold_px": float(getattr(params_module, "STAGE5_GT_DEDUPE_DIST_PX", 2.0)),
        }

    params_module = _load_python_module(
        "_tmp_baseline_night_params",
        _night_pipeline_dir() / "pipeline_params.py",
    )
    validator_module = _load_python_module(
        "_tmp_baseline_night_stage9_validate",
        _night_pipeline_dir() / "stage9_validate.py",
    )
    return {
        "route": "night",
        "validator_module": validator_module,
        "crop_w": int(getattr(params_module, "STAGE9_CROP_W", 10)),
        "crop_h": int(getattr(params_module, "STAGE9_CROP_H", 10)),
        "gt_area_threshold_px": int(getattr(params_module, "AREA_THRESHOLD_PX", 4)),
        "gt_bright_max_threshold": int(getattr(params_module, "BRIGHT_MAX_THRESHOLD", 50)),
        "min_pixel_brightness_for_area_calc": int(
            getattr(params_module, "MIN_PIXEL_BRIGHTNESS_TO_BE_CONSIDERED_IN_AREA_CALCULATION", 50)
        ),
        "gt_dedupe_dist_threshold_px": float(getattr(params_module, "STAGE9_GT_DEDUPE_DIST_PX", 2.0)),
    }


def _rows_to_gt_by_t(rows: Sequence[Dict[str, int]]) -> Dict[int, List[Tuple[int, int]]]:
...
def _gt_by_t_to_rows(gt_by_t: Dict[int, List[Tuple[int, int]]]) -> List[Dict[str, int]]:
    out: List[Dict[str, int]] = []
    for t in sorted(gt_by_t.keys()):
        for x, y in gt_by_t.get(t) or []:
            out.append({"x": int(x), "y": int(y), "t": int(t)})
    return out


def _normalize_gt_rows_for_baseline(
    *,
    route: str,
    video_path: Path,
    gt_rows: Sequence[Dict[str, int]],
    max_frames: Optional[int],
) -> tuple[List[Dict[str, int]], Dict[str, Any]]:
    video_frames = _video_frame_count(video_path)
    inferred_gt_t_offset = _infer_gt_t_offset_from_rows(video_path=video_path, gt_rows=gt_rows)
    manual_gt_t_shift = int(BASELINE_GT_T_SHIFT_BY_VIDEO_STEM.get(video_path.stem, 0))
    frame_limit = video_frames
    if max_frames is not None:
        frame_limit = min(frame_limit, int(max_frames)) if frame_limit is not None else int(max_frames)
    ...
    for row in gt_rows:
        try:
            x = int(row["x"])
            y = int(row["y"])
            t = int(row["t"]) - int(inferred_gt_t_offset) + int(manual_gt_t_shift)
        except Exception:
            continue
        if t < 0:
            continue
        if frame_limit is not None and t >= int(frame_limit):
            continue
        normalized_rows.append({"x": x, "y": y, "t": int(t)})

    route_config = _baseline_gt_processing_config(route)
    validator_module = route_config["validator_module"]
    gt_by_t = _rows_to_gt_by_t(normalized_rows)
    gt_by_t_filtered, kept_after_filter, total_before_filter = validator_module._filter_gt_by_brightness_and_area(
        video_path,
        gt_by_t,
        crop_w=int(route_config["crop_w"]),
        crop_h=int(route_config["crop_h"]),
        bright_max_threshold=int(route_config["gt_bright_max_threshold"]),
        area_threshold_px=int(route_config["gt_area_threshold_px"]),
        max_frames=frame_limit,
        area_min_pixel_brightness=int(route_config["min_pixel_brightness_for_area_calc"]),
    )
    gt_by_t_dedup, removed_duplicates = validator_module._dedupe_gt_via_distance_and_weight(
        video_path,
        gt_by_t_filtered,
        crop_w=int(route_config["crop_w"]),
        crop_h=int(route_config["crop_h"]),
        dist_threshold_px=float(route_config["gt_dedupe_dist_threshold_px"]),
        max_frames=frame_limit,
    )

    processed_rows = _gt_by_t_to_rows(gt_by_t_dedup)
    normalized_gt_max_t = max((int(r["t"]) for r in processed_rows), default=None)
    return processed_rows, {
        "gt_processing_mode": "route_internal_filter_and_dedupe",
        "gt_processing_route": str(route_config["route"]),
        "video_frames": (int(video_frames) if video_frames is not None else None),
        "raw_gt_rows": int(len(gt_rows)),
        "offset_normalized_gt_rows": int(len(normalized_rows)),
        "filtered_gt_rows": int(kept_after_filter),
        "filter_input_gt_rows": int(total_before_filter),
        "dedupe_removed_gt_rows": int(removed_duplicates),
        "normalized_gt_rows": int(len(processed_rows)),
        "raw_gt_max_t": max((int(r["t"]) for r in gt_rows), default=None),
        "normalized_gt_max_t": (int(normalized_gt_max_t) if normalized_gt_max_t is not None else None),
        "inferred_gt_t_offset": int(inferred_gt_t_offset),
        "manual_gt_t_shift": int(manual_gt_t_shift),
        "crop_w": int(route_config["crop_w"]),
        "crop_h": int(route_config["crop_h"]),
        "gt_area_threshold_px": int(route_config["gt_area_threshold_px"]),
        "gt_bright_max_threshold": int(route_config["gt_bright_max_threshold"]),
        "min_pixel_brightness_for_area_calc": int(route_config["min_pixel_brightness_for_area_calc"]),
        "gt_dedupe_dist_threshold_px": float(route_config["gt_dedupe_dist_threshold_px"]),
    }
...
def _safe_ultralytics_local_weights_path(weights: Path) -> Path:
    """Copy local weights into a safe cache path when the source path trips Ultralytics parsing."""
    s = str(weights)
    if "'" not in s:
        return weights
    ULTRALYTICS_WEIGHTS_CACHE_ROOT.mkdir(parents=True, exist_ok=True)
    safe_path = ULTRALYTICS_WEIGHTS_CACHE_ROOT / weights.name
...
def _resolve_ultralytics_init_weights(weights: str | Path) -> Path:
    """
    Resolve init weights outside the repo CWD so bare names like `yolov8s.pt`
    do not get downloaded into the project root.
    """
    raw = Path(str(weights).strip())
    candidates = [raw] if raw.is_absolute() else [raw, REPO_ROOT / raw]
    for candidate in candidates:
        if candidate.exists():
            return _safe_ultralytics_local_weights_path(candidate.resolve())

    if (not raw.is_absolute()) and len(raw.parts) == 1 and raw.suffix.lower() == ".pt":
        from ultralytics.utils.downloads import attempt_download_asset

        ULTRALYTICS_WEIGHTS_CACHE_ROOT.mkdir(parents=True, exist_ok=True)
        cached = Path(str(attempt_download_asset(ULTRALYTICS_WEIGHTS_CACHE_ROOT / raw.name)))
        return cached
...
def _train_one_day_yolo_model(
    *,
    ...
    init_weights = _resolve_ultralytics_init_weights(DAY_YOLO_MODEL_WEIGHTS_INIT)
...
def _metrics_table_fields_from_summary(summary: str) -> Dict[str, Any]:
    ...
    tp = _int_or_none(parsed.get("tp"))
    fp = _int_or_none(parsed.get("fp"))
    fn = _int_or_none(parsed.get("fn"))
    if tp is None or fp is None or fn is None:
        out["status"] = "error"
        out["error"] = f"unable to parse metric summary: {s}"
        return out

    precision = _float_or_none(parsed.get("precision"))
    accuracy = _float_or_none(parsed.get("accuracy"))
    f1 = _float_or_none(parsed.get("f1"))
    recall = (float(tp) / float(tp + fn)) if (tp + fn) > 0 else 0.0
...
def _build_pipeline_inference_results_table(rows: Sequence[Dict[str, str]]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for row in rows:
        eval_type = str(row.get("eval_type") or "")
        if eval_type not in {"global", "leaveout", "video_skip"}:
            continue
        metrics_fields = _metrics_table_fields_from_summary(str(row.get("results") or ""))
        out.append(
            {
                "eval_type": eval_type,
                ...
                "gt_rows": _int_or_none(row.get("gt_rows")),
                "gt_max_t": _int_or_none(row.get("gt_max_t")),
                **metrics_fields,
            }
        )
    return out
...
def _build_baseline_inference_results_table(rows: Sequence[Dict[str, str]]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for row in rows:
        if str(row.get("eval_type") or "") != "baseline":
            continue
        common = {
            "route": str(row.get("route") or ""),
            "species_name": str(row.get("species_name") or ""),
        ...
        for method_key, field_name in (("lab", "lab_results"), ("raphael", "raphael_results")):
            summary = str(row.get(field_name) or "").strip()
            if not summary:
                continue
            metrics_fields = _metrics_table_fields_from_summary(summary)
            out.append(
                {
                    **common,
                    "method_key": method_key,
                    "method_name": _baseline_method_display_name(method_key),
                    **metrics_fields,
                }
            )
    return out
...
def _patch_training_status(metrics: Dict[str, Any], *, dry_run: bool) -> str:
    if not metrics:
        return "dry_run" if dry_run else "empty"
    if metrics.get("error"):
        return "error"
    if any(metrics.get(k) is not None for k in ("best_epoch", "best_val_acc", "test_loss", "test_acc")):
        return "ok"
    return "empty"


def _build_patch_training_results_table(
    *,
    dataset_summaries: Dict[str, Dict[str, Dict[str, Any]]],
    training_metrics: Dict[str, Dict[str, Dict[str, Any]]],
    dry_run: bool,
) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for route in ("day", "night"):
        route_metrics = dict(training_metrics.get(route) or {})
        route_dataset_summaries = dict(dataset_summaries.get(route) or {})
        for model_key in sorted(route_metrics.keys()):
            metrics = dict(route_metrics.get(model_key) or {})
            dataset_summary = dict(route_dataset_summaries.get(model_key) or {})
            left_out_species = model_key[len("leaveout_") :] if model_key.startswith("leaveout_") else ""
            out.append(
                {
                    "route": route,
                    "model_key": model_key,
                    "model_scope": ("leaveout" if left_out_species else "global"),
                    "left_out_species": left_out_species,
                    "trained_on_species": _trained_on_species_from_dataset_summary(dataset_summary),
                    "status": _patch_training_status(metrics, dry_run=dry_run),
                    "best_epoch": _int_or_none(metrics.get("best_epoch")),
                    "best_val_acc": _float_or_none(metrics.get("best_val_acc")),
                    "test_loss": _float_or_none(metrics.get("test_loss")),
                    "test_acc": _float_or_none(metrics.get("test_acc")),
                }
            )
    return out


def _day_yolo_training_status(results_summary: Dict[str, Any], *, dry_run: bool) -> str:
    if not results_summary:
        return "dry_run" if dry_run else "empty"
    if results_summary.get("error"):
        return "dry_run" if str(results_summary.get("error")) == "dry_run" else "error"
    if results_summary.get("final_row") or results_summary.get("rows_logged"):
        return "ok"
    return "empty"


def _build_day_yolo_training_results_table(
    *,
    training_manifests: Dict[str, Dict[str, Any]],
    dry_run: bool,
) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for model_key in sorted(training_manifests.keys()):
        manifest = dict(training_manifests.get(model_key) or {})
        results_summary = dict(manifest.get("results_summary") or {})
        final_row = results_summary.get("final_row")
        out.append(
            {
                "model_key": model_key,
                "model_scope": ("leaveout" if str(manifest.get("leaveout_species") or "").strip() else "global"),
                "left_out_species": str(manifest.get("leaveout_species") or ""),
                "species_included": [
                    str(species_name)
                    for species_name in list(manifest.get("species_included") or [])
                    if str(species_name or "").strip()
                ],
                "status": _day_yolo_training_status(results_summary, dry_run=dry_run),
                "rows_logged": _int_or_none(results_summary.get("rows_logged")),
                "final_metrics": (dict(final_row) if isinstance(final_row, dict) else {}),
            }
        )
    return out


def _build_run_results_manifest(
    *,
    run_id: str,
    dry_run: bool,
    ...
    pipeline_table = _build_pipeline_inference_results_table(rows)
    baseline_table = _build_baseline_inference_results_table(rows)
    patch_training_table = _build_patch_training_results_table(
        dataset_summaries=dataset_summaries,
        training_metrics=training_metrics,
        dry_run=dry_run,
    )
    day_yolo_training_table = _build_day_yolo_training_results_table(
        training_manifests=day_yolo_training_manifests,
        dry_run=dry_run,
    )
    return {
        "manifest_version": "tmp_run_results_manifest_v1",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "run_id": str(run_id),
        "dry_run": bool(dry_run),
        "summary": {
            "pipeline_inference_rows": int(len(pipeline_table)),
            "baseline_inference_rows": int(len(baseline_table)),
            "patch_training_rows": int(len(patch_training_table)),
            "day_yolo_training_rows": int(len(day_yolo_training_table)),
        },
        "inference": {
            "pipeline_inference_table": pipeline_table,
            "baseline_inference_table": baseline_table,
        },
        "training": {
            "patch_classifier_training_table": patch_training_table,
            "day_yolo_training_table": day_yolo_training_table,
        },
    }
...
def _run_baseline_gt_matcher(
    *,
    ...
    if not matcher_script.exists():
        raise FileNotFoundError(f"baseline GT matcher script not found: {matcher_script}")
    cmd = [
        sys.executable,
        str(matcher_script),
        "--video",
...
def _run_baselines_for_video(
    *,
    run_root: Path,
    video: EvalVideo,
    baseline_gt_rows: Sequence[Dict[str, int]],
    logs_dir: Path,
    run_lab: bool,
    run_raphael: bool,
    dry_run: bool,
    ...
    if run_lab:
        ...
        if not dry_run:
            ...
            _write_gt_csv(gt_csv, baseline_gt_rows)
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "with baselines using internal route-aware GT processing."
        )
    )
    p.add_argument(
        "--raw-videos-root",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    results_manifest_path = run_root / RUN_RESULTS_MANIFEST_FILENAME
    ...
    run_lab_baseline = bool(RUN_LAB_BASELINE)
    run_raphael_baseline = bool(RUN_RAPHAEL_BASELINE)
    run_baselines = bool(run_lab_baseline or run_raphael_baseline)
    ...
    if (not run_baselines) and (not run_any_pipeline) and (not bool(RUN_MODEL_TRAINING)) and (not run_day_yolo_training):
        raise SystemExit(
            "Nothing to run: RUN_MODEL_TRAINING=False, RUN_DAY_YOLO_MODEL_TRAINING=False, "
            "RUN_LAB_BASELINE=False, RUN_RAPHAEL_BASELINE=False, "
    ...
    baseline_gt_rows_by_video_key: Dict[str, List[Dict[str, int]]] = {}
    baseline_gt_by_video_key: Dict[str, Dict[str, Any]] = {}
    if run_baselines:
        for vid in baseline_eval_videos:
            normalized_gt_rows, baseline_gt_info = _normalize_gt_rows_for_baseline(
                route=vid.route,
                video_path=vid.video_path,
                gt_rows=vid.gt_rows,
                max_frames=_video_max_frames_from_gt(vid),
            )
            baseline_gt_rows_by_video_key[vid.video_key] = normalized_gt_rows
            baseline_gt_by_video_key[vid.video_key] = {
                **baseline_gt_info,
    ...
    if run_baselines:
        print(
            "[baselines] using internal route-aware GT processing from raw video annotations; "
            "no pipeline inference toggles are required for baselines."
        )
    ...
    if run_baselines:
        ...
        for i, vid in enumerate(baseline_eval_videos, start=1):
            ...
            baseline_gt_info = baseline_gt_by_video_key.get(vid.video_key) or {}
            baseline_gt_rows = baseline_gt_rows_by_video_key.get(vid.video_key) or []
            if not baseline_gt_rows:
                baseline_by_video_key[vid.video_key] = {
                    mk: {
                        "metrics": {"error": "baseline GT normalization produced no rows for this video"},
                        "out_root": "",
                        "rendered_video": "",
                    }
                ...
                baseline_by_video_key[vid.video_key] = _run_baselines_for_video(
                    run_root=run_root,
                    video=vid,
                    baseline_gt_rows=baseline_gt_rows,
                    logs_dir=logs_dir,
            ...
            _record_row(
                {
                    ...
                    "raphael_results": raphael_metrics,
                    "raphael_output_path": raphael_path,
                    "gt_source": str(baseline_gt_info.get("gt_source") or vid.gt_source or ""),
                    "gt_rows": str(baseline_gt_info.get("normalized_gt_rows") or 0),
                    "gt_max_t": str(baseline_gt_info.get("normalized_gt_max_t") or ""),
                }
            )
        if not dry_run:
            _write_baseline_species_results_and_registry(
                ...
                baseline_gt_by_video_key=baseline_gt_by_video_key,
                dry_run=dry_run,
            )
    ...
    results_manifest = _build_run_results_manifest(
        run_id=run_id,
        dry_run=dry_run,
        rows=rows_out,
        dataset_summaries=dataset_summaries,
        training_metrics=training_metrics,
        day_yolo_training_manifests=day_yolo_training_manifests,
    )
    if not dry_run:
        results_manifest_path.write_text(json.dumps(results_manifest, indent=2), encoding="utf-8")
        print(f"[tmp-run] results manifest: {results_manifest_path}")
    ...
    record: Dict[str, Any] = {
        ...
        "baseline_gt_by_video_key": dict(baseline_gt_by_video_key),
