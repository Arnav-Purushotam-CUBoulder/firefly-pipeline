...
def run_for_video(video_path: Path) -> Path:
    ...
    with out_csv.open("w", newline="") as f:
        wcsv = csv.writer(f)
        wcsv.writerow(["x", "y", "w", "h", "frame_number"])  # centers
        ...
        for idx, t in enumerate(frames_sorted):
            ...
            for row in by_frame[t]:
                ...
                dx = float(new_cx - cx)
                dy = float(new_cy - cy)
                disp_all.append(float(np.hypot(dx, dy)))
                refined_count += 1
                per_frame_counts[t] = per_frame_counts.get(t, 0) + 1
                ...
                out_name = f"t{t:06d}_x{int(round(new_cx))}_y{int(round(new_cy))}_{w}x{h}.png"
                cv2.imwrite(str(crops_dir / out_name), crop_c)

                wcsv.writerow([float(new_cx), float(new_cy), int(w), int(h), int(t)])
            if idx % 20 == 0:
    ...
    cap.release()
    # --- Stats summary ---
    frames_in_range = len(frames_sorted)
    print(f"Stage3  Refined patches: {refined_count}")
    print(f"Stage3  Frames considered: {frames_in_range}")
    if disp_all:
        arr = np.asarray(disp_all, dtype=np.float32)
        print(
            f"Stage3  Refinement shift(px): min={arr.min():.2f} p25={np.percentile(arr,25):.2f} "
            f"p50={np.percentile(arr,50):.2f} mean={arr.mean():.2f} p75={np.percentile(arr,75):.2f} max={arr.max():.2f}"
        )
    if per_frame_counts:
        counts = np.asarray([per_frame_counts.get(t, 0) for t in frames_sorted], dtype=np.int32)
        print(
            f"Stage3  Positives/frame: min={int(counts.min())} p50={np.percentile(counts,50):.1f} "
            f"mean={counts.mean():.2f} max={int(counts.max())}"
        )
        nz = [(t, int(per_frame_counts.get(t, 0))) for t in frames_sorted if per_frame_counts.get(t, 0) > 0]
        nz.sort(key=lambda x: (-x[1], x[0]))
        top = nz[:5]
        if top:
            top_s = ", ".join([f"t={t}:{c}" for (t, c) in top])
            print(f"Stage3  Top frames by refined: {top_s}")
